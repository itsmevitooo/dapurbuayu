    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        @forelse($reviews as $review)
            <div class="bg-white p-6 rounded-2xl shadow-xl border-b-4 border-primary font-inter flex flex-col h-full transition-all hover:shadow-2xl">
                ...
                <div class="flex mb-3">
                    @for($i = 1; $i <= 5; $i++)
                        <svg class="w-5 h-5 {{ $i <= (int)$review->rating ? 'text-yellow-400' : 'text-gray-200' }}" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div class="flex flex-wrap gap-2 mb-6">
                        @foreach($images as $img)
                            ...
                            <div class="w-20 h-20 overflow-hidden rounded-xl border border-gray-100 shadow-sm">
                                <img src="{{ asset('storage/' . ltrim($img, '/')) }}" class="w-full h-full object-cover" onerror="this.src='https://placehold.co/100x100?text=No+Image'">
                            </div>
                            @endif
                        @endforeach
                    </div>
